...
class SklearnModel:
    ...
    def train(self, train_npz, selected_npz, params_file):

        X, y, featnames = u.read_data(train_npz, selected_npz)
    ...
    def predict_proba(self, test_npz, selected_npz):

        X_test, _, _ = u.read_data(test_npz, selected_npz)

        y_proba = self.clf.predict_proba(X_test)
        u.save_proba_npz(y_proba, self.best_hyperparams)
        
    def predict(self, test_npz, selected_npz):
